...
BENCH_DIR="$ROOT_DIR/workspace_microbench"
BENCH_SRC="$BENCH_DIR/bingo_stride.c"
BENCH_BIN="$BENCH_DIR/bingo_stride.x86"
...
run_case() {
    local name="$1"; shift
    local outdir="$BENCH_DIR/out_${name}"
    rm -rf "$outdir"
...
main() {
    ...
    echo "Outputs in workspace_microbench/out_no_bingo, out_bingo_l2, out_bingo_l2_multi"
}
